...
OC_SYNC_FILE="$tmp/openclaw.json" OC_SYNC_PORT=18789 OC_SYNC_BIND=lan OC_SYNC_TOKEN=test-token \
	node "$tmp/sync.js"
...
if(d.gateway.port!==18789||d.gateway.bind!=="lan"||d.gateway.auth.token!=="test-token")process.exit(2);
