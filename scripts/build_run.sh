...
install_files() {
	...
	mkdir -p "$dest/www/luci-static/resources/openclaw" "$dest/www/luci-static/resources/view/openclaw"
	...
	cp "$PKG_DIR/root/usr/share/rpcd/ucode/luci.openclaw" "$dest/usr/share/rpcd/ucode/"
	chmod +x "$dest/usr/share/rpcd/ucode/luci.openclaw"
...
for dep in luci-base rpcd-mod-ucode curl openssl-util script-utils tar ttyd qrencode; do
...
Package: $PKG
Version: $PKG_VER
Depends: luci-base, rpcd-mod-ucode, curl, openssl-util, script-utils, tar, ttyd, qrencode, libstdcpp6
Section: luci
Architecture: all
Installed-Size: 0
...
# 清理旧版 Lua Controller/CBI/View，防止旧路由与现代 menu.d 同时存在。
