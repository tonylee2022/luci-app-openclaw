...
function setRawMode(enable) {
  ...
  // 持久按键监听随 rawMode 一并装卸: 避免每次 readKey 反复 attach/detach 'data'
  // 监听导致 stdin 在两次按键间暂停, 进而出现"每个键需按两次才响应"的问题。
  if (enable) {
    installKeyListener();
  } else {
    removeKeyListener();
...
/**
 * 解析一个输入序列为按键对象
 * @returns {{name: string, sequence: string, char: string, paste?: string}}
 */
function parseKeySeq(seq) {
  let key = { sequence: seq, char: '', name: '' };

  // 检测 Bracketed Paste (粘贴): ESC[200~ <内容> ESC[201~
  if (seq.startsWith('\x1b[200~') && seq.endsWith('\x1b[201~')) {
    key.name = 'paste';
    key.paste = seq.slice(6, -6);
    return key;
  }
  ...
  else if (seq === '\x7f' || seq === '\x08') key.name = 'backspace';
  else if (seq === '\x1b[A') key.name = 'up';
  else if (seq === '\x1b[B') key.name = 'down';
  else if (seq === '\x1b[C') key.name = 'right';
  else if (seq === '\x1b[D') key.name = 'left';
  else if (seq === '\x1b[1;2A' || seq === '\x1b[1;2B') key.name = seq.endsWith('A') ? 'shift-up' : 'shift-down';
  else if (seq === '\x1b[5~') key.name = 'pageup';
  else if (seq === '\x1b[6~') key.name = 'pagedown';
  else if (seq === '\x1b[H') key.name = 'home';
  else if (seq === '\x1b[F') key.name = 'end';
  else if (seq.length === 1 && seq >= ' ' && seq <= '~') { key.char = seq; key.name = seq.toLowerCase(); }
  else if (seq.startsWith('\x1b[') && seq.endsWith('~')) {
    const code = seq.slice(2, -1);
    if (code >= '11' && code <= '24') key.name = `f${parseInt(code) - 10}`;
  }

  return key;
}

// ── 持久按键监听 + 队列: stdin 在整个 rawMode 期间保持流动, 不再每键暂停/恢复 ──
let _keyQueue = [];
...
function installKeyListener() {
  ...
  _keyResolver = null;
  _keyHandler = (buffer) => {
    const key = parseKeySeq(buffer.toString('utf8'));
    if (_keyResolver) {
      const resolve = _keyResolver;
      _keyResolver = null;
      resolve(key);
    } else {
      _keyQueue.push(key);
...
function removeKeyListener() {
  if (_keyHandler) {
    process.stdin.off('data', _keyHandler);
    _keyHandler = null;
  }
  _keyQueue = [];
  _keyResolver = null;
