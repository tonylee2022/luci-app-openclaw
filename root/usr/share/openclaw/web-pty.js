...
const OC_HOME = process.env.OC_HOME || installPath;
const OC_GLOBAL = process.env.OC_GLOBAL || installPath + '/.npm-global';
const OC_DATA = process.env.OC_DATA || OC_HOME;
const OC_STATE_DIR = process.env.OPENCLAW_STATE_DIR || OC_HOME + '/.openclaw';
const OC_NPM_CACHE = process.env.NPM_CONFIG_CACHE || OC_HOME + '/.npm';
const OC_TMP = process.env.TMPDIR || OC_HOME + '/.tmp';
...
class PtySession {
  constructor(socket, initCmd = '') {
    ...
    this._setupWSReader();
    this._startPing();
    // 延迟到收到首个 resize 再创建 PTY, 以真实终端尺寸 spawn。
    // 否则 PTY 固定 80x24, 与实际更宽的 xterm 不符 → TUI 重绘清行算错 → 重影/重复显示、光标错位(看似"按键不响应")。
    // 兜底: 400ms 内没收到 resize 则用默认尺寸 spawn。
    this._spawnFallback = setTimeout(() => { if (!this._spawned && this.alive) this._spawnPty(); }, 400);
  }
  ...
  _handleMessage(text) {
    try {
      ...
      if (msg.type === 'stdin' && this.proc && this.proc.stdin.writable) {
        ...
        if (!this._spawned) {
          this._spawnPty();  // 首个 resize: 以真实尺寸创建 PTY
        } else if (this.proc && this.proc.pid) {
