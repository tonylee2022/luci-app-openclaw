...
if command -v oc_load_paths >/dev/null 2>&1 && oc_load_paths "$OC_CONFIGURED_PATH"; then
	...
	OC_STATE_DIR="${OC_STATE_DIR:-${OC_HOME}/.openclaw}"
	OC_NPM_CACHE="${OC_NPM_CACHE:-${OC_HOME}/.npm}"
		...
		if confirm_yes "$install_qqbot"; then
			...
			if [ $install_rc -eq 0 ]; then
				...
				if [ -d "$qqbot_ext_dir" ] && [ -f "${qqbot_ext_dir}/openclaw.plugin.json" ]; then
					...
					echo -e "  ${CYAN}插件是否启用及允许加载由 OpenClaw 官方配置决定。${NC}"
					echo -e "  ${CYAN}请根据上方官方错误信息检查插件配置。${NC}"
					plugin_installed=1
				else
...
health_check() {
	...
	# 使用 gateway health --json 做深度健康检查。
...
case "${1:-}" in
	...
	--model)
		...
		;;
	--configure-model)
		# 模型/提供商配置 (LuCI 配置管理页嵌入终端)。
		# 有真实 PTY 时用方向键交互菜单; 无 PTY (多数 OpenWrt 的 web 终端) 自动用行式菜单。
		# 行式菜单内含「官方完整向导」子选项, 供具备 PTY 的环境使用。
		if can_use_interactive; then
			launch_interactive_model_config
		else
			configure_model
		...
		;;
	--configure-channels)
		# 渠道配置: configure_channels 为行式交互 (echo + read), 无需 PTY, 内含官方向导子选项。
		configure_channels
		;;
	--set)
		if [ -n "${2:-}" ] && [ -n "${3:-}" ]; then
