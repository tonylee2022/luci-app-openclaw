...
const OC_HOME = process.env.OC_HOME || OC_INSTALL_PATH;
const OC_GLOBAL = process.env.OC_GLOBAL || `${OC_INSTALL_PATH}/.npm-global`;
    ...
    switch (choice.value) {
      case 'create-config': {
        ...
        try {
          ...
          console.log(`${C.yellow}⚠️ 备份命令执行失败${C.reset}\n`);
        }
        await input({ prompt: '按回车继续', defaultValue: '' });
        break;
      }
